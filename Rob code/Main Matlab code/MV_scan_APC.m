...
for iii = 1:2                                  % Number of trajectories that will be ran (set to 25)
...
jjj         = 0.10;                             % Fraction of agonist pMHC molecules
...
CF          = 1000/60;                          % Conversion factor         [µm/min] -> [nm/s]
tf          = 1.0;                              % Final time point of the simulation    [s]
...
rho_pMHC    = 1*100;                            % Density of pMHC molecules on APC      [1/µm^2]
...
Bond_distr_VSV8_0_10_T = Bond_distr;       % Record bond lifetime distribution data
Bond_total_VSV8_0_10_T = Bond_total;       % Record number of bonds
MV_center_VSV8_0_10_T  = MV_center;        % Record microvillus center displacement data
MV_Force_VSV8_0_10_T   = F_MV;             % Record the forces in the simulation
Velocity_VSV8_0_10_T   = V_MV;             % Record microvillus velocity for the trajectory

% Clear unnecessary variables from workspace
clear Ag_neighbor aRand aTemp aTot Bond_distr Bond_total BondCounter Box_area ...
    ...
    Diss_no Dist_Ag Dist_Catch Dist_E1 Dist_Slip Dist_TCR E1_neighbor F_MV fc_VSV8 ...
    ...
    kon_std ks_VSV8 ll Lx Ly max_x max_y min_x min_y MV_center MV_Force PAg_diff ...
    ...
    theta time V_MV Velocity VFSampleCT VFsampleRate x_mv x_MV0 x_new x_trial ...
    y_new y_trial z_bond;

% Create a filename and save the workspace of the trajectory
filename = ['strong_slip_' num2str(jjj) '_E1_' num2str(1-jjj) '_04_04_T' num2str(iii) '.mat'];
save(filename);

display(iii);                                   % Output trajectory number to screen
